angular.module('app.ganister.tabs.node.nodeFormTabs.externalAPIs.gitlab.listCommits', [
  'app.ganister.externalAPIs.gitlab',
  'ui-notification',
]).controller('externalAPIsGitlabListCommitsController', ($rootScope, $scope, Notification, gitlabModel) => {
  ...
  $scope.fetchData = async () => {
    ...
    const { data, error, message } = await gitlabModel.commits.listCommits($scope.node.properties[owner], $scope.node.properties[repo]);
  ...
  $scope.gridOptions = {
    ...
    columnDefs: [{
      ...
      field: "url",
      cellRenderer: params => {
        return `<a href="${params.data.html_url}" target="_blank">view on gitlab</a>`;
      }
    }],
    rowHeight: 50,
    rowData: [],
